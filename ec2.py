...
class Ec2Inventory(object):
    ...
    def read_settings(self):
        ...
        if (configRegions == 'all'):
            if self.eucalyptus_host:
                self.regions.append(
                    boto.connect_euca(
                        host=self.eucalyptus_host).region.name,
                    **self.credentials)
            else:
        ...
        if self.all_instances:
            ...
            for instance_state in config.get(
                    'ec2', 'instance_states').split(','):
        ...
        if config.has_option('ec2', 'all_rds_instances') and self.rds_enabled:
            self.all_rds_instances = config.getboolean(
                'ec2', 'all_rds_instances')
        else:
            self.all_rds_instances = False

        # Return all ElastiCache replication groups? (if ElastiCache is
        # enabled)
        if config.has_option(
                'ec2', 'all_elasticache_replication_groups') and self.elasticache_enabled:
        ...
        if config.has_option(
                'ec2', 'all_elasticache_clusters') and self.elasticache_enabled:
        ...
        if config.has_option(
                'ec2', 'all_elasticache_nodes') and self.elasticache_enabled:
        ...
        if config.has_option('ec2', 'instance_filters'):
            ...
            filters = [
                f for f in config.get(
                    'ec2',
                    'instance_filters').split(',') if f]

            for instance_filter in filters:
                instance_filter = instance_filter.strip()
                if not instance_filter or '=' not in instance_filter:
                    continue
                filter_key, filter_value = [x.strip()
                                            for x in instance_filter.split('=', 1)]
                if not filter_key:
                    continue
    ...
    def connect_to_aws(self, module, region):
        ...
        # only pass the profile name if it's set (as it is not supported by
        # older boto versions)
        if self.boto_profile:
    ...
    def get_instances_by_region(self, region):
        ...
        try:
            ...
            # reliable and may be missing, and the only way to guarantee they
            # are there is by calling `get_all_tags`
            instance_ids = []
    ...
    def get_auth_error_message(self):
        ...
        if None in [os.environ.get('AWS_ACCESS_KEY_ID'), os.environ.get(
        ...
        boto_config_found = list(
            p for p in boto_paths if os.path.isfile(
    ...
    def add_instance(self, instance, region):
        ...
        if self.group_by_tag_keys:
            for k, v in instance.tags.items():
                ...
                for v in values:
                    ...
                    if self.nested_groups:
                        self.push_group(
                            self.inventory, 'tags', self.to_safe(
                                "tag_" + k))
                        if v:
                            self.push_group(
                                self.inventory, self.to_safe(
                                    "tag_" + k), key)
        ...
        self.inventory["_meta"]["hostvars"][
            hostname]['ansible_ssh_host'] = dest
    ...
    def add_rds_instance(self, instance, region):
        ...
        if self.group_by_rds_engine:
            ...
            if self.nested_groups:
                self.push_group(
                    self.inventory, 'rds_engines', self.to_safe(
                        "rds_" + instance.engine))
    ...
    def add_elasticache_cluster(self, cluster, region):
        ...
        if not self.all_elasticache_clusters and cluster[
                'CacheClusterStatus'] != 'available':
            return

        # Select the best destination address
        if 'ConfigurationEndpoint' in cluster and cluster[
        ...
        if self.group_by_availability_zone and not is_redis:
            ...
            if self.nested_groups:
                if self.group_by_region:
                    self.push_group(self.inventory, region, cluster[
                                    'PreferredAvailabilityZone'])
                self.push_group(self.inventory, 'zones', cluster[
                                'PreferredAvailabilityZone'])
        ...
        if self.group_by_security_group and not is_redis:
            ...
            if 'SecurityGroups' in cluster and cluster[
                    'SecurityGroups'] is not None:
        ...
        if self.group_by_elasticache_parameter_group:
            ...
            if self.nested_groups:
                self.push_group(
                    self.inventory, 'elasticache_parameter_groups', self.to_safe(
                        cluster['CacheParameterGroup']['CacheParameterGroupName']))
    ...
    def add_elasticache_node(self, node, cluster, region):
        ...
        if not self.all_elasticache_nodes and node[
    ...
    def add_elasticache_replication_group(self, replication_group, region):
        ...
        # Only want available clusters unless
        # all_elasticache_replication_groups is True
        ...
        # Inventory: Group by availability zone (doesn't apply to replication
        # groups)
    ...
    def get_host_info_dict_from_describe_dict(self, describe_dict):
        ...
        for key in describe_dict:
            ...
            if key == 'ec2_node_groups' and value:
                host_info['ec2_endpoint_address'] = value[
                    0]['PrimaryEndpoint']['Address']
                host_info['ec2_endpoint_port'] = value[
                    0]['PrimaryEndpoint']['Port']
                replica_count = 0
